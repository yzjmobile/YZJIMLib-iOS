...
#define PACKAGE_PROPERTY_IS_GZIB	0x1	/// 这是GZIB压缩
		...
		const std::string const_cgi_open_session = "/open_session";
		const std::string const_cgi_close_session = "/close_session";
		const std::string const_cgi_login = "/login";
		const std::string const_cgi_send_msg = "/send_msg";
		const std::string const_cgi_public_send_msg = "/public/send_msg";
#endif
...
#if (USES_MERC_FUNC==1)
enum {
	kEctLocalSessionID = -20,
	kEctLocalBufParam = -21,
	kEctAESDecryptFail = -30,
	kEctGzipUnZipFail = -40,
};
#endif
...
enum NetStatus {
    ...
    kServerDown = 5
#if (USES_MERC_FUNC==1)
	, kLonklinkConnectFailed = 19		/// 长连连接失败，后台和网络主动断开情况不处理（重复失败，只上传一次）
	, kHeartBeatFailed = 20				/// 心跳失败，后台和网络主动断开情况不处理
	, kFailedReConnected = 21			/// 失败重连成功
#endif
};
